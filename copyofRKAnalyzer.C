...
void RKAnalyzer::ElectronProducer(){
 ...
 Zp4 = (*fourmom1) + (*fourmom2);
    ...
    if(nEle >=2 && triggerstatus && electrons.etSC > 25. && fabs(electrons.etaSC) < 2.5 && !(fabs(electrons.etaSC) > 1.4442 && fabs(electrons.etaSC) < 1.566 ) && (*eleIsPassMedium)[i]==1 && electrons.EcalDrivenSeed ) RKElectronCollection.push_back(electrons);
  ...
  if(nEle >= 2){
       ...
       if(triggerstatus){
         ...
         if(kinematic >= 2){
		   ...
		   std::cout << "run:lumi:event " << runId << ":"<<lumiSection<<":"<<eventId<<" "<< Zp4.M() <<std::endl;   
		 }   
	      }             
	    }
	 }
