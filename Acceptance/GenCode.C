...
void GenCode(){
  ...
  while (std::getline(inputFile, str))
    ...
    for (int i=0; i < numfiles; i++){
      ...
      tree->SetBranchAddress("genStFlag",&genStFlag);
      //tree->SetBranchAddress("genParId",&genParId);
      tree->SetBranchAddress("genParSt",&genParSt); 
      //tree->SetBranchAddress("genParP4",&genParP4);
      tree->SetBranchAddress("nGenPar",&nGenPar);
	...
	//	vector<int> &GenStFlag = *((vector<int>*) tree.GetPtr("genStFlag"));

        for (Int_t ele = 0; ele < nGenPar ; ele++) { 
	  if(genStFlag[ele] >> 1 && genParSt[ele] ==1){
         
	  cout << "nGenPar: " <<  ele << "  genStFlag :   " << genStFlag[ele] << "   Par Status :   " << genParSt[ele]  << std::endl;
	}
	}
	//        if(nVtx >0){
	//pileup->Fill(nVtx);}
