{
  ...
  //histoname
  TString histnameString("h_ZMass_HEEPID");
  TFile *fIn;
  ...
  for(int i =0; i< nfiles  ; i++){
    ...
    h_den[i] = (TH1F*) fIn->Get("h_ZMass_Acc_Gen");
    if(i==1 && false){
    ...
    std::cout<<" scale factor "<<i <<"  "<<Xsec[i]/h_lum->Integral()<<std::endl;
    std::cout<<" bin 42 "<<h_num[i]->GetBinContent(42)<<std::endl;
    std::cout<<" bin 42 error "<<h_num_error[i]->GetBinContent(42)<<std::endl;
    
    h_num[i]->Scale(Xsec[i]/h_lum->Integral());
    h_den[i]->Scale(Xsec[i]/h_lum->Integral());
    
    
    // Raman
    h_num_error[i]->Scale(Xsec[i]/h_lum->Integral());
    h_den_error[i]->Scale(Xsec[i]/h_lum->Integral());
    // Raman ---
    ...
    //h_num[i]->Sumw2();
    //h_den[i]->Sumw2();
    std::cout << filenameString[i] << "   " << " h_ZMass_HEEPID "  <<   " Integral : " << h_num[i]->Integral() << "   " << h_den[i]->Integral() << std::endl;
  }
  
  std::cout<<" now outside i loop "<<std::endl;
  ...
  std::cout<<" total error for bin 42 = "<<Stackhist_num_error->GetBinContent(42)<<std::endl;
  ...
  histo->Draw("P");
  histo->SetName("h_ZMass_HEEPID_h_ZMass_Acc_Gen");
  
  //TGraphAsymmErrors* gr = new TGraphAsymmErrors(Stackhist_num, Stackhist_den);
  //gr->Draw("AP");
  //gr->SetName("h_ZMass_HEEPID_h_ZMass_Acc_Gen");
  
  TString FilePreName("AccEff_All");
  TFile *fout= new TFile("AccEff_All2.root","UPDATE");
