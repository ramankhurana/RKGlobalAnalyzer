...
//histoname
 TString histnameString("Electron_NoCut_/h_nVtx");
 if(0){TString histnameString("h_nVtx");}
...
// Main Code starts here
 ...
 Int_t binxmax = xaxis->FindBin(50);


//std::mout << " Range given from  "  << 0 <<" to  " << 50 << "with bin number from :  "<< binxmin << " to  :  " << binxmax <<std::endl; 
...
// For N-1 Plots only
 bool nminus = 0;
 TLatex *tt;
 if(("Electron_NoCut_" == "ElectronNMinus1E") || ("Electron_NoCut_" == "ElectronNMinus1B") ){
   nminus =1;
...
//
  ...
  hs->GetXaxis()->SetTitle("# of vertices");
  }else{
  ...
  hs->GetXaxis()->SetRangeUser(0,50);                                                                                                      hs->GetXaxis()->SetNdivisions(508); 
  legend->Draw("same"); 
  t2a->Draw("same");
  t2b->Draw("same");
  t2c->Draw("same");
 ...
 // Lower Tpad Decalaration
  ...
  DataMC->GetXaxis()->SetTitle("# of vertices");
//DataMC->GetXaxis()->SetIndiceSize(0.1);
  ...
  DataMC->GetXaxis()->SetRangeUser(0,50);
  DataMC->Draw("PE1");
...
//=========================================================================
 ...
 c12->Draw();
 c12->SaveAs(DirPreName+dirpathname +"/DYPdf/Electron_NoCut__h_nVtx.pdf");
 c12->SaveAs(DirPreName+dirpathname +"/DYPng/Electron_NoCut__h_nVtx.png");
 c12->SaveAs(DirPreName+dirpathname +"/DYROOT/Electron_NoCut__h_nVtx.root");                                                                         }
