...
void plot_Asymptotic_ForCombination(string outputdir)
{
  bool obs=false;
...
//  gROOT->LoadMacro("CMS_lumi.C");
  ...
  // double fr_left = 590.0, fr_down = 1E-5, fr_right = 2000.0, fr_up = 0.5; 
   double fr_left = 590.0, fr_down = 5E-5, fr_right = 2600.0, fr_up = 5;
...
//    leg->AddEntry(grthSM10, "#sigma_{TH} x BR(Z' #rightarrow " + VV + "), #tilde{k}=0.20", "L"); // #rightarrow 2l2q
  ...
  gPad->SetLogy();
  sprintf(fnam, "MonoHbbLimit%s_Asymptotic_log.png", outputdir.data());
    cMCMC->SaveAs(fnam);
  /*
