...
TString filenamepath("MonoHFatJetAnalysis_AllMCWithPU_V3/");
...
//---By Raman
 h_mc_error[i] = (TH1F*) fIn->Get(histnameString);
//--Raman **
...
//---By Raman
 h_mc_error[i]->Rebin(REBIN);
 int nbins = h_mc_error[i]->GetNbinsX();
 for( int j=0; j < nbins-1; j++){
      float NMC   = h_mc_error[i]->GetBinContent(j+1); 
      h_mc_error[i]->SetBinContent(j+1,TMath::Sqrt(NMC));
      h_mc_error[i]->Scale(normalization[i]);

}
     std::cout<< h_mc[5]->Integral();
//---Raman 
