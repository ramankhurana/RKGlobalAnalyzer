...
listsame=FileToList('ListOfEventsOnlyIn75.txt')
...
#    idx74 = FindIndex(list74,listsame[isameE][0])
 #   if idx74 ==-1 : continue
    ...
    if idx75 ==-1 : continue
    mass.append([list75[idx75][1]])
    pt.append([list75[idx75][2]])
    rapidity.append([list75[idx75][3]])
    phi.append([list75[idx75][4]])
    leadpt.append([list75[idx75][5]])
    subleadpt.append([list75[idx75][6]])
    leadeta.append([list75[idx75][7]])
    subleadeta.append([list75[idx75][8]])
    leadphi.append([list75[idx75][9]])
    subleadphi.append([list75[idx75][10]])
...
SaveHisto(mass,['mass75',60,60,120,'M_{ee}'])
SaveHisto(pt,['pt75',30,0,300,'p_{T}^{ee}'])
SaveHisto(rapidity,['rapidity75',48,-2.4,2.4,'Rapidity_{ee}'])
SaveHisto(phi,['phi75',70,-3.5,3.5,'#phi_{ee}'])
SaveHisto(leadpt,['leadpt75',60,0,300,'p_{T}^{e}(leading)'])
SaveHisto(subleadpt,['subleadpt75',60,0,300,'p_{T}^{e}(sub-leading)'])
SaveHisto(leadeta,['leadeta75',48,-2.4,2.4,'#eta_{e}(leading)'])
SaveHisto(subleadeta,['subleadeta75',48,-2.4,2.4,'#eta_{e}(sub-leading)'])
SaveHisto(leadphi,['leadphi75',70,-3.5,3.5,'#phi_{e}(leading)'])
SaveHisto(subleadphi,['subleadphi75',70,-3.5,3.5,'#phi_{e}(sub-leading)'])
