{
  ...
  TFile *f1 = TFile::Open("Data_246908-260627_80mb.root");
  TH1F *data = (TH1F*) f1->Get("pileup");
  data->Scale(1./data->Integral());
  data->SetNameTitle("pileup","pileup" );
  TFile *f2 = new TFile("DataPU_80mb.root", "recreate");
  f2->cd();
  data->Write();
  f2->Close();

  /*  TFile *f3 = TFile::Open("MC_trueInt.root");
  TH1F *mc = (TH1F*) f3->Get("h");
  mc->Scale(1./mc->Integral());
  mc->SetNameTitle("pileup","pileup" );
  TFile *f4 = new TFile("MCPU_trueInt.root", "recreate");
  f4->cd();
  mc->Write();
  f4->Close();
  */
