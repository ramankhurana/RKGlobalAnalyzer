...
//by  b_branchname->GetEntry(ientry); //read only this branch
  ...
  const int nbins = 19;
  float bins[8]={0.,50.,100.,150.,200.,
		  250.,300.,2000.};
  
  n_events = new TH1F("n_events","",10,0,10);
  MET_N    = new TH1F("MET_N","",7,bins);
  MET_D    = new TH1F("MET_D","",7,bins);
  n_jet    = new TH1F("n_jet","",5,0,5);
  if (fChain == 0) return;
   ...
   for (Long64_t jentry=0; jentry<nentries;jentry++) {
     ...
     //MET_N->Fill(pfMetCorrPt);
     //MET_N->Fill(pfMetRawPt);
     //bool mettrigger = TriggerStatus("HLT_PFMET170_NoiseCleaned_");
     //if(mettrigger) MET_D->Fill(pfMetCorrPt);
     //if(mettrigger) MET_D->Fill(pfMetRawPt);
	...
	if(njet==2)        {
      ...
      if(njet<1) continue;
      if(!(p41.Pt()>110)) continue;
      if(clean1)   n_events->Fill(2);
...
bool Sync::TriggerStatus(std::string TRIGNAME){
